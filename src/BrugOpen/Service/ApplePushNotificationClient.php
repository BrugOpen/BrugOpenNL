        ...
        if ($this->endpointUrl == null) {
            ...
            if ($this->context) {
                ...
                if ($config) {
                    ...
                    $configParam = trim((string)$config->getParam('applepushdispatcher.url'));
        ...
        if ($this->bearerToken == null) {
            ...
            if ($this->context) {
                ...
                if ($config) {
                    ...
                    $configParam = trim((string)$config->getParam('applepushdispatcher.token'));
    ...
    public function dispatchMessages($messages, $batchSize = 100)
    {
        ...
        if ($endpointUrl) {
            ...
            foreach ($batches as $batch) {
                ...
                $log->info('Dispatching ' . count($batch) . ' apple push message' . (count($batch) != 1 ? 's' : ''));
            ...
            $log->error('No applepushdispatcher.url set in config');
        }
    }
}
